...
mnhttn_sls_clean$bldg_cat <- gsub(" ","", mnhttn_sls_clean$BUILDING.CLASS.CATEGORY, fixed=TRUE)

require(memisc) #Enable recode.
mnhttn_sls_clean <- within(mnhttn_sls_clean, {
    nhood <- recode(nhood, "c('WASHINGTONHEIGHTSLOWER',
                              'WASHINGTONHEIGHTSUPPER')='WASHINGTONHEIGHTS';
                            c('UPPERWESTSIDE(59-79)',
                              'UPPERWESTSIDE(79-96)',
                              'UPPERWESTSIDE(96-116)')='UWS';
                            c('UPPEREASTSIDE(59-79)',
                              'UPPEREASTSIDE(79-96)',
                              'UPPEREASTSIDE(96-110)')='UES';
                            c('HARLEM-WEST',
                              'HARLEM-UPPER',
                              'HARLEM-EAST',
                              'HARLEM-CENTRAL')='HARLEM';
                            c('GREENWICHVILLAGE-WEST',
                              'GREENWICHVILLAGE-CENTRAL')='GREENWICHVILLAGE'")
})

#Code categorical variables.
mnhttn_sls_clean <- within(mnhttn_sls_clean, {
    nhood <- factor(nhood)
    bldg_cat <- factor(bldg_cat)
...
s <- sample(nrow(subset_mnhttn)/2, nrow(subset_mnhttn)/4)
train_mnhttn <- subset_mnhttn[s,] 
test_mnhttn <- subset_mnhttn[-s,]

train_mnhttn_vbls <- train_mnhttn[,1:length(subset_mnhttn)-1]
test_mnhttn_vbls <- test_mnhttn[,1:length(subset_mnhttn)-1]
train_mnhttn_nhood <- train_mnhttn[,length(subset_mnhttn)]
test_mnhttn_nhood <- test_mnhttn[,length(subset_mnhttn)]

#Implement K-nearest neighbors.
predict_nhood <- knn(train_mnhttn_vbls, test_mnhttn_vbls, train_mnhttn_nhood, k=5)

#Calculate percentage accuracy of classifier.
1-(sum(predict_nhood != test_mnhttn_nhood)/length(test_mnhttn_nhood))
